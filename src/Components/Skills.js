...
const Skills = ({ skillsData, bodyClassName }) => {
    ...
    return (
      <div className={`skills ${backgroundColor} text-2xl p-8 flex sm:flex-row flex-col items-center justify-center`}>
      <h2 className={`text-3xl ${headerTextColor} font-bold sm:w-4/12`}>{skillsData.titles[0]}</h2>
      <div className="grid grid-cols-2 gap-4 m-0 mt-4">
        {skillsData.skills.map((skill) => (
          <div key={skill.id} className={`flex sm:flex-row flex-col items-center p-4 ${skillsTextColor} text-lg m-0`}>
